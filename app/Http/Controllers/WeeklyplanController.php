...
class WeeklyplanController extends Controller
{
    ...
    public function index(Request $request, $teamid)
    {
        ...
        $plan = Plan::where('plantype', 'weekly')->where('teamid', $teamid)->orderby('plans.created_at', 'desc')->where('sessionid', $session->id)
         ...
         if($userf != 0){
          $plan = Plan::where('plantype', 'weekly')->where('teamid', $teamid)->orderby('plans.created_at', 'desc')->where('sessionid', $session->id)->where('plans.userid',$userf)
            ->join('users', 'users.id', '=', 'plans.userid')->select('plans.id', 'plans.updated_at', 'users.fname','users.lname','users.avatarcolor', 'plans.userid', 'users.team', 'plans.created_at', 'plans.reportsTo', 'plans.cc')->paginate(5)
            ->appends('user', $userf);
       }
        
         $count = Plan::where('plantype', 'weekly')->where('teamid', $teamid)->orderby('plans.created_at', 'desc')->where('sessionid', $session->id)
        ...
        return view('weeklyplan', compact(['db', 'plan' , 'team', 'objectives', 'existplan', 'users','currentPath', 'userf', 'count']));
    }
     public function edit($id)
    {
        ...
        //fetch all keyresutls in the database
        $metric = Keyresult::all();
         return view('editweeklyplan', compact(['db', 'plan' , 'team', 'objectives', 'users']));
    }
 public function show($id)
    ...
    public function store(Request $request, $teamid)
    {
        ...
        /* Add plan */
        DB::transaction(function () use ($teamid,$taskids,$input, $request) {
        $plan = new Plan;
        ...
        $plan = Plan::where("userid", Auth::user()->id)->where('plantype', 'weekly')->where('teamid', $teamid)->where('sessionid', Session::where('status', 'Active')->first()->id)->orderby('updated_at', 'desc')->where('sessionid', Session::where('status', 'Active')->first()->id)->first();
    ...
    public function comment($plan_id){
        ...
        return view('weeklycomment', compact(['plan','message','comments', 'type' , 'team','currentPath']));
    }
