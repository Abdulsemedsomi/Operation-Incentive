...
<div class="container">
                                                    ...
                                                    <div class="col-md-3">
                                                        ...
                                                        <div class="col-md-3">
                                                            <label class="css-control-sm css-control-success css-radio mt-5">
                                                                <input type="radio" class="css-control-input" name="radio-group{{$task->id}}" onchange="taskstatuscheck({{$task->id}})"  value=1>
                                                                <span class="css-control-indicator"></span> Achieved
                                                            </label>
                                                            <label class="css-control-sm css-control-danger css-radio mt-5">
                                                                <input type="radio" class="css-control-input" name="radio-group{{$task->id}}" checked value=0 onchange="taskstatuscheck({{$task->id}})">
                                                                <span class="css-control-indicator"></span> Not Yet
                                                            </label>
                                                        </div>
                                                        <div class="col-md-3">
                                                            <input type="text" class="form-control round" placeholder="Reason" name="feedback{{$task->id}}" id="tfeedback{{$task->id}}" required >
                                                        </div>
                                                    @else
                                                    <div class="col-md-3">
                                                        <label class="css-control-sm css-control-info css-checkbox css-checkbox-rounded">
                                                        ...
                                                        <div class="container">
                                                            <div class="container ml-10">
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-md-3">
                                                                        <label class="css-control-sm css-control-success css-radio mt-5">
                                                                            <input type="radio" class="css-control-input" name="subradio-group{{$subtask->id}}" onchange="substatuscheck({{$subtask->id}})"  value=1>
                                                                            <span class="css-control-indicator"></span> Achieved
                                                                        </label>
                                                                        <label class="css-control-sm css-control-danger css-radio mt-5">
                                                                            <input id="failedsubradio-group{{$subtask->id}}" type="radio" class="css-control-input" name="subradio-group{{$subtask->id}}" onchange="substatuscheck({{$subtask->id}})"  value=0 checked>
                                                                            <span class="css-control-indicator"></span> Not Yet
                                                                        </label>
                                                                    </div>
                                                                   
                                                                    <div class="col-md-3">
                                                                        <input type="text" class="form-control round" placeholder="Reason" name="subfeedback{{$subtask->id}}" id="subfeedback{{$subtask->id}}" required>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
    ...
    <div class="container neopad">
                                    ...
                                    <ol type="1">
                                        @foreach($weekplan as $wp)
                                            @if($wp->keyresultid == $mv->keyresultid)
                                                @php
                                                    $subtasks =  DB::table('dailyreports')->where('report_id', $p->id)->where('dailyreports.task_id', $wp->id)->join('subtasks','dailyreports.subtask_id','=', 'subtasks.id')->get();
                                                @endphp
                                                @if($subtasks->count() == 0)
                                                    <div class = "row">
        ...
        @endif
            ...
            @if( Auth::user()->position == "CEO" || Auth::user()->id == 170 || Auth::user()->id == 86 ||  Auth::user()->id == 183  || (Auth::user()->team != $team->team_name && Gate::any(['fillkpiteam', 'fillengageteam'])))
                 <div class="mt-20" >
                ...
                if(data == 1){
                      ...
                      $("#resutlcbut" + planid).html("&#128077;" + " " + comment);
                        if(comment == "Noted with concern") {
                         ...
                         document.getElementById('submitbutton'+planid).innerHTML = '1 comment';
