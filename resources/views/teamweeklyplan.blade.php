    ...
    <div class="col-md-8">
     ...
     <div  class="container neopad addweeklancard" hidden>
         <div class="card text-center" >
             <div class="card-body text-left">
                    ...
                    <form action = "{{ route('tweeklyplans', ['teamid'=>$team->id, 'sessionid'=>$session->id]) }}" id="submitweeklyplan"  method="post">
                        @csrf
                        ...
                        @foreach($objectives as $objs)
                                ...
                                <div class="card">
                                    <div class="card-header" id="weeklyheading{{$m->id}}" style="font-size: 14px;">
                                        <div class="row ">
                                            ...
                                            <p class=" mt-5 col-md-4"><i class="si si-flag mr-10 text-success"></i>{{App\Task::where('isMilestone', 1)->where('keyresultid', $m->id)->where('status', 0)->get()->count()}} Milestones</p>
                                            ...
                                            <a type="button" class="text-success mt-5  col-md-4 addweeklytask {{App\Task::where('isMilestone', 1)->where('keyresultid', $m->id)->where('status', 0)->get()->count() > 0?'':'ancdisabled'}}" id="addweeklytask{{$m->id}}" data-id = '{{$m->id}}'><i class="fa fa-plus mr-5"></i> Add Task
                            ...
                            <div class="card-body">
                                ...
                                <button type="submit" class="btn btn-rounded btn-outline-primary submitweekplan" onclick="Codebase.loader('show', 'bg-gd-sea');setTimeout(function () { Codebase.loader('hide'); }, 3500);">Submit Weekly target</button>
                                <div class="col-md-1"></div>
                                <button type="button" class="btn btn-rounded btn-outline-danger cancelweeklyplan" onclick="Codebase.loader('show', 'bg-gd-sea');setTimeout(function () { Codebase.loader('hide'); }, 3500);">Cancel</button>
                                </div>
                            </div>
                    </form>
             </div>
     ...
     <div class="col text-center">
        ...
        @endphp
         ...
         <button type="button"  class="btn btn-rounded btn-outline-info min-width-125 mb-10 addweekplanbutton" {{$isdisabled }} {{$session->status =="Closed"?"hidden":""}}>Add Next Week's target</button>
     </div>

     <div  class="container neopad infinite-scroll">
        @foreach($plan as $p)        
        @php
            ...
            <div class="row pull-right">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" style="">
                        <a class="dropdown-item" href="{{route('tweeklyplan.edit',  ['teamid'=>$team->id, 'sessionid'=>$session->id, 'planid'=>$p->id])}}" onclick="Codebase.loader('show', 'bg-gd-sea');setTimeout(function () { Codebase.loader('hide'); });">
      ...
      @endforeach
        ...
        <div class="card-body">
            <div class="row pull-right">
           <a type="submit" class="btn btn-rounded btn-outline-info min-width-125" href="{{route('tweeklycomment', ['teamid'=>$team->id, 'sessionid'=>$session->id, 'planid'=>$p->id])}}" onclick="Codebase.loader('show', 'bg-gd-sea');setTimeout(function () { Codebase.loader('hide'); });">
            @if($comments > 1)
            {{$comments}} comments
            @elseif($comments > 0)
            {{$comments}} comment
