        ...
        <div class="card text-center" >
            <div class="card-body text-left">
                <form action = "{{ route('dailyplans', $team->id) }}"   method="post">
                    @csrf
                     <!--<input type="hidden"  name="weekplanid" value= "">-->
...
<div class=" row">
    ...
    <button type="button"  class="btn btn-rounded btn-outline-info min-width-125 mb-10 adddailyplanbutton " {{$isdisabled}}>Add Today's target</button>
    </div>
...
@if($ncomments)
        ...
        <div class="card-body">
            <div class="row pull-right">
           <a type="submit" class="btn btn-rounded btn-outline-info min-width-125" href="{{route('tdailycomment', ['teamid'=>$team->id, 'sessionid'=>$session->id, 'planid'=>$p->id])}}" id="submitbutton{{$p->id}}">
            @if($comments > 1)
            {{$comments}} comments
            @elseif($comments > 0)
            {{$comments}} comment
