        ...
        @foreach($plan as $p)
            ...
            <div class="card text-center ribbon ribbon-success">
                ...
                <div class="card-body text-left">
                        ...
                        <div class="form-group ">
                            @foreach($failuretargets as $f)
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                       <label  class="form-control">{{ App\Weeklyreport::where('report_id', $p->id)->where('failurereason_id', $f->failurereason_id)->get()->count() > 0 ? App\Weeklyreport::where('report_id', $p->id)->where('failurereason_id', $f->failurereason_id)->get()->sum('task_target'):0}}</label>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    <label  class="text-center ">Total: <b class="{{ $taskachieved >= 85 ? 'text-success' : ($taskachieved <= 60 ? 'text-danger' : 'text-warning') }}">{{$taskachieved}}% </b></label>
                    @if($p->cc !=null)
                    ...
                    @endif
                    <!--If boost has been given already-->
                    @php
                        $result = 'Noted';
                        if($taskachieved >= 85){
                            $result = 'Good Job';
                        }
                        else if($taskachieved <= 60){
                            $result = 'Noted with concern';
                        }
                    @endphp
                    @if($ncomments)
                        @php
                        ...
                        @if(Auth::user()->id == $p->reportsTo || Auth::user()->id == 170)
                            ...
                            <div class="container" id="boostdiv{{$p->id}}" style="display:none">
                                ...
                                <div class="row pull-right" onclick="addcomment('{{$p->id}}', '{{$result}}')">
                ...
                <div class="card-body">
                    <div class="row pull-right">
                        <a type="submit" class="btn btn-rounded btn-outline-info min-width-125" href="{{route('weeklyrcomment', $p->id)}}" id="submitbutton{{$p->id}}">
                            @if($comments > 1)
                                {{$comments}} comments
                            @elseif($comments > 0)
                                {{$comments}} comment
...
function addcomment(planid, comment) {
   ...
   $.ajaxSetup({
        ...
        var ajaxurl = baseurl + "weeklyrncomment";
